...
下面的图是 Mermaid 源码，GitHub 会将其渲染为可缩放的 SVG。完整源码保存在 [`plugins/northline/docs/architecture.mmd`](./plugins/northline/docs/architecture.mmd)，可以直接复制到 Mermaid 编辑器继续修改。
...
sequenceDiagram
    participant Root
    participant Northline as Northline Control Plane
    participant Worker
    participant Leaf
    participant Verifier
    participant Git as Git Worktree

    Root->>Northline: 初始化 MissionState 与 ProtocolPolicy
    Root->>Northline: 创建并审查 DelegationContract v1
    Northline->>Git: 从 base_commit 创建隔离 worktree
    Northline->>Worker: 发送版本化 AgentTaskPacket
    Worker->>Northline: CLAIMED → EXECUTING
    Worker->>Leaf: 仅在契约授权范围内派生子任务
    Leaf->>Git: 修改允许文件并提交结果 commit
    Leaf->>Worker: HandoffReceipt + 测试证据
    Worker->>Northline: REPORTING
    Northline->>Verifier: 重算 diff、祖先关系、范围和测试
    Verifier-->>Northline: VERIFIED 或 REJECTED
    Northline->>Root: 返回可审查结果与风险
    Root->>Git: 审查后执行唯一集成
    Root->>Northline: 记录全局测试和 INTEGRATED
```
